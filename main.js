...
const addNewTask = (e) => {
  ...
  if (!value) return alert("Task title cannot be empty!");

  if (hasDuplicate(value)) {
    inputEl.value = "";
    return alert(`"${value}" is already existed. Please try another title!`);
  }
...
const renderTask = () => {
  const html = tasks
    .map(
      (task, index) => `
    <div class="js-item flex items-center justify-between py-3 px-4 my-4 rounded-md bg-[#8758ff] text-white" data-index=${index}>
        <p>${htmlEntities(task.title)}</p>
